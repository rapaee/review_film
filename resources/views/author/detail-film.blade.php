...
<body>
    ...
    {{-- <section class="bg-center w-full bg-no-repeat bg-cover -mt-6 bg-[url('{{ asset('storage/' . $datafilm->poster) }}')] bg-gray-700 bg-blend-multiply"> --}}
        <a href="{{ route('author.film') }}" class="flex items-center absolute border py-2 px-5 rounded-lg ml-5 hover:bg-gray-100">
        ...
        <div class="px-4 mx-auto max-w-screen-xl h-[500px] text-center py-24 lg:py-56">
            ...
            <div class="flex flex-col items-center md:flex-row -mt-0 md:-mt-36 md:max-w-9/12">
                <img class="object-cover w-full rounded-t-lg h-96 md:h-[350px] md:w-72 md:rounded-none md:rounded-s-lg" src="{{ asset('storage/' . $datafilm->poster) }}" alt="">
                <div class="flex flex-col justify-start p-4 md:p-10 leading-normal">
                    <h5 class="mb-2 text-2xl font-bold text-left tracking-tight uppercase text-gray-900 dark:text-black">{{ $datafilm->judul }}</h5>
                                ...
                                <div class="bg-white p-6 rounded-md w-96 relative">
                                    ...
                                    <form id="castingForm" action="{{ route('author.detail-film.casting') }}" method="POST">
                                        ...
                                        <div class="mb-4">
                                            ...
                                            <input type="text" id="nama_panggung" name="nama_panggung" class="mt-1 p-2 w-full border border-gray-300 rounded-md" required>
                                        </div>
                            
                                        <div class="mb-4">
                                            <label for="nama_asli" class="block text-sm font-medium text-gray-700 text-left">Nama Asli</label>
                                            <input type="text" id="nama_asli" name="nama_asli" class="mt-1 p-2 w-full border border-gray-300 rounded-md" required>
                                        </div>
                                    ...
                                    <button @click="open = false" class="absolute top-2 right-2 text-gray-500 hover:text-gray-700 z-10">
