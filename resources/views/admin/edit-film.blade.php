...
<form class="max-w-full mx-auto p-5" method="POST" action="{{ route('admin.edit-film.update', $film->id_film) }}" enctype="multipart/form-data">
    ...
    <div class="mb-3">
        ...
        <input type="file" id="trailer" name="trailer" accept="video/*" class="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-100 dark:border-gray-600 dark:placeholder-gray-400 dark:text-black dark:focus:ring-blue-500 dark:focus:border-blue-500 dark:shadow-sm-light " value="{{ old('trailer',$film->trailer) }}" required />
        @error('trailer')
        <span class="text-red-700">{{ $message }}</span>
       @enderror
    </div>
    <div class="flex justify-end gap-4">
        <a href="{{ route('admin.film') }}" class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-red-600 dark:hover:bg-red-700 dark:focus:ring-blue-800">Kembali</a>
